...
- `current` Direct adapter-to-SQLite streaming with transactional rollback; canonical payloads are not materialized as a second temporary file.
- `current` Bounded 1,000-event local pages so typical long rollouts open in one request while raw-byte ceilings still cap payload size.
