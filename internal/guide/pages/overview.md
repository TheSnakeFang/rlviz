...
The hosted browser directly supports reviewed `.rlviz` bundles, RLViz canonical NDJSON, Letta trajectory v1 JSON, Harbor ATIF v1.5-v1.7, Inspect AI EvalLog JSON, and Verifiers GenerateOutputs JSON. A selected browser directory must contain exactly one supported `.ndjson` or `.json` trace export; open an `.rlviz` bundle through the file picker or drag it onto the viewer. Complete Harbor job directories and bundle creation use the native CLI.

We highly recommend installing the local CLI. It opens complete Harbor jobs as one read-only source. Coding agents can operate its workspaces and build adapters or plugins for private formats, larger cohorts, and other multi-file datasets. Browser adapters run as reviewed WASM modules; local adapters can use the source system's native files and libraries.
