...
export const flows: Flow[] = [
  ...
  },
  {
    id: "y", name: "local-metadata-edit-filter-and-restore", keyboardOnly: false, surfaces: ["daemon", "webapp"], webappExample: "Terminal-Bench 2.0 · reviewed cohort", steps: [
      ...
      { action: { kind: "reload" }, expect: [{ target: "rail", selector: ".editable-metadata h1", equals: "Checkout reliability" }, { target: "read", selector: ".lane-track header b", equals: "Reviewed checkout" }, { target: "console", selector: ".workspace-console .annotation-tags", contains: "reviewedauth" }] },
    ],
    webappSteps: [
      { action: { kind: "click", target: ".workspace-rail .metadata-edit" }, expect: [{ target: "rail", selector: ".metadata-editor.collection" }] },
      { action: { kind: "fill", target: "[aria-label='collection title']", value: "Checkout reliability" }, expect: [{ target: "rail", selector: "[aria-label='collection title']", value: "Checkout reliability" }] },
      ...
      { action: { kind: "fill", target: "[aria-label='trajectory description']", value: "Confirmation path under review" }, expect: [{ target: "console", selector: "[aria-label='trajectory description']", value: "Confirmation path under review" }] },
      { action: { kind: "fill", target: "[aria-label='trajectory labels']", value: "reviewed, auth" }, expect: [{ target: "console", selector: "[aria-label='trajectory labels']", value: "reviewed, auth" }] },
      { action: { kind: "click", target: ".workspace-console .metadata-editor button[type='submit']" }, expect: [{ target: "read", selector: ".lane-track header b", equals: "Reviewed checkout" }, { target: "console", selector: ".workspace-console .editable-metadata p", equals: "Confirmation path under review" }, { target: "console", selector: ".workspace-console .annotation-tags", contains: "reviewedauth" }] },
      { action: { kind: "key", value: "Escape" }, expect: [{ target: "browse" }] },
      { action: { kind: "filter", value: "auth" }, expect: [{ target: "selected-row", contains: "Reviewed checkout" }] },
      { action: { kind: "key", value: "Escape" }, expect: [{ target: "browse" }, { target: "selected-row", contains: "Reviewed checkout" }] },
      { action: { kind: "reload" }, expect: [{ target: "browse", attribute: "data-filter", equals: "auth" }, { target: "selected-row", contains: "Reviewed checkout" }] },
    ],
  },
  {
  ...
  {
    id: "aa", name: "indexed-query-filter-and-pagination", keyboardOnly: false, surfaces: ["daemon"], steps: [
      { action: { kind: "filter", value: "checkpoint:ckpt-7 status:failed" }, expect: [attr("browse", "data-filter", "checkpoint:ckpt-7 status:failed")] },
      { action: { kind: "key", value: "Enter" }, expect: [{ target: "rail", selector: "[role='status']", contains: "indexed cohort2 matches · $0.0200" }, { target: "selected-row", contains: "indexed-1" }, { target: "rail", selector: ".editable-metadata small", contains: "1/2 trajectories loaded" }] },
      { action: { kind: "click", target: ".rail-actions button:has-text('load more')" }, expect: [{ target: "browse", selector: "[role='option']", count: 2 }, { target: "browse", selector: "[role='option']:has-text('indexed-2')" }, { target: "rail", selector: ".editable-metadata small", contains: "2/2 trajectories loaded" }] },
    ],
  },
];
